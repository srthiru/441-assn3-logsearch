...
```bash
test-app$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
test-app$ sam local start-api
...
```bash
test-app$ cd HelloWorldFunction
...
```bash
aws cloudformation delete-stack --stack-name test-app
```
